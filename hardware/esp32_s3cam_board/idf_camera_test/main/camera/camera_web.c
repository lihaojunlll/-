...
static const char INDEX_HTML[] =
    ...
    "header{padding:14px;background:#1d1d1d}.viewer{position:relative;display:inline-block;max-width:100%}"
    "img{display:block;max-width:100%;height:auto;background:#000;filter:grayscale(1)}canvas{position:absolute;inset:0;width:100%;height:100%;pointer-events:none}"
    ...
    "<header><h2>ESP32-S3 OV3660 Grayscale Camera</h2></header>"
    "<div class='wrap'><div class='viewer'><img id='view' src='/stream'><canvas id='trail'></canvas></div>"
    "<div class='status'>"
    "<div class='box'><div class='label'>Route</div><div class='value' id='turn'>WAIT</div></div>"
    "<div class='box'><div class='label'>Slowdown</div><div class='value' id='slow'>0.00</div></div>"
    "<div class='box'><div class='label'>Quality</div><div class='value' id='quality'>0.00</div></div>"
    "<div class='box'><div class='label'>Curve</div><div class='value' id='curve'>0.00</div></div>"
    "<div class='box'><div class='label'>Near / Mid / Far / Look</div><div class='value' id='rows'>--</div></div>"
    ...
    "function drawTrail(s){let img=document.getElementById('view'),c=document.getElementById('trail');"
    "let w=img.clientWidth,h=img.clientHeight;if(!w||!h)return;if(c.width!==w)c.width=w;if(c.height!==h)c.height=h;"
    "let ctx=c.getContext('2d');ctx.clearRect(0,0,w,h);let rows=[['look',s.look,s.look_y],['far',s.far,s.far_y],['mid',s.mid,s.mid_y],['near',s.near,s.near_y]];"
    "ctx.lineWidth=1;ctx.strokeStyle='rgba(255,255,255,.28)';ctx.fillStyle='rgba(255,255,255,.72)';ctx.font='12px Arial';"
    "for(let r of rows){let y=py(r[2],h,s.frame_height);ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(w,y);ctx.stroke();ctx.fillText(r[0],4,Math.max(12,y-3));}"
    "ctx.lineWidth=4;ctx.strokeStyle=s.turn<0?'#ffcc33':(s.turn>0?'#55b8ff':'#8ee66f');ctx.beginPath();"
    "for(let i=0;i<rows.length;i++){let x=px(rows[i][1],w),y=py(rows[i][2],h,s.frame_height);if(i===0)ctx.moveTo(x,y);else ctx.lineTo(x,y);}ctx.stroke();"
    "for(let r of rows){let x=px(r[1],w),y=py(r[2],h,s.frame_height);ctx.beginPath();ctx.arc(x,y,5,0,6.283);ctx.fillStyle='rgba(0,0,0,.65)';ctx.fill();ctx.lineWidth=3;ctx.stroke();}}"
    ...
    "else{t.textContent='STRAIGHT';t.className+='turn-straight'}"
    ...
    "document.getElementById('rows').textContent=f(s.near)+','+f(s.mid)+','+f(s.far)+','+f(s.look);"
    ...
    "drawTrail(s);"
    "}catch(e){document.getElementById('turn').textContent='ERR'}setTimeout(poll,200)}poll();"
    "</script>"
    "</div></body></html>";
...
void camera_web_init(camera_config_t *config)
{
    ...
    if (sensor != NULL) {
        ...
        ESP_LOGI(TAG, "force grayscale: special_effect=%d saturation=%d",
